...
class GameResult(models.Model):
    ...
    winner = models.ForeignKey(
        Player, on_delete=models.CASCADE, default=None, blank=True, null=True
    )
    ...
    def __str__(self):
        ...
        return result.format(
            self.id,
            self.game.name,
            self.winner.name if self.winner is not None else "nobody",
            self.str_status(),
        )
...
class GameResultDetail(models.Model):
    ...
    def __str__(self):
        ...
        return detail.format(
            self.result.id, self.result.game.name, self.player.name, self.points
        )
